...
namespace Texture_Editor.Halo2Beta
{
    ...
    public partial class TextureEditor : AbideTool
    {
        ...
        private void TextureEditor_SelectedEntryChanged(object sender, EventArgs e)
        {
            //Filter bitmaps
        ...
        {
            //Check
            if (this.bitmap != null) bitmap_Load((int)bitmapUpDown.Value, (int)lodUpDown.Value - 1);
        }
        ...
        private void exportToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            if (save)
            {
                ...
                bool swizzled = ((BitmapFlags)bitmap.Flags & BitmapFlags.PowTwoDimensions) == BitmapFlags.PowTwoDimensions && 
                    bitmap.Format != HaloBitmap.BitmapFormat.Dxt1 && bitmap.Format != HaloBitmap.BitmapFormat.Dxt3 && bitmap.Format != HaloBitmap.BitmapFormat.Dxt5;
        ...
        private void bitmap_Load(int bitmapIndex, int lodIndex)
        {
            ...
            if ((bitmap.Bitmaps[bitmapIndex].RawOffsets[lodIndex] & (uint)HaloBitmap.RawLocation.SinglePlayerShared) != 0)
        ...
        private bool bitmap_ImportImage(Image image, int bitmapIndex)
        {
            ...
            //Create Data Buffer
            byte[] buffer = image_CreateBuffer(image, out bool linear, out bool swizzle, out bool deleteLods);
            ...
            if (swizzle) flags |= HaloBitmap.BitmapFlags.PeferStutter;
            ...
            bitmap.Bitmaps[bitmapIndex].PixelsOffset = 0;
            bitmap.Bitmaps[bitmapIndex].RegistrationPointX = (short)(image.Width / 2);
            bitmap.Bitmaps[bitmapIndex].RegistrationPointY = (short)(image.Height / 2);
        ...
        private bool bitmap_ImportDds(DdsFile ddsFile, int bitmapIndex)
        {
            ...
            //Check
            if(ddsFile.MipmapCount > 9)
            {
                MessageBox.Show("Mipmap count exceeds the maximum. Halo 2 Beta only supports up to 9 mipmap levels.", "Mipmap count exceeded", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            ...
            bitmap.Bitmaps[bitmapIndex].Flags = flags;
            bitmap.Bitmaps[bitmapIndex].PixelsOffset = 0;
            bitmap.Bitmaps[bitmapIndex].RegistrationPointX = (short)(ddsFile.Width / 2);
            bitmap.Bitmaps[bitmapIndex].RegistrationPointY = (short)(ddsFile.Height / 2);
        ...
        private byte[] image_CreateBuffer(Image image, out bool linear, out bool swizzle, out bool deleteLods)
        {
            ...
            //Get Linear
            linear = Math.Log(image.Width, 2) % 1 != 0 || Math.Log(image.Height, 2) % 1 != 0;
            swizzle = !linear;

            //Check
            if (swizzle) buffer = Swizzler.Swizzle(buffer, image.Width, image.Height, 1, 32, !swizzle);
        ...
        private byte[][] ddsFile_CreateBuffer(DdsFile ddsFile, out HaloBitmap.BitmapFormat format, ref int lodLevels, out bool linear, out bool swizzle, out bool deleteLods)
        {
            ...
            {
                swizzle = Math.Log(ddsFile.Width, 2) % 1 == 0 && Math.Log(ddsFile.Height, 2) % 1 == 0;
                linear = !swizzle;
            }
